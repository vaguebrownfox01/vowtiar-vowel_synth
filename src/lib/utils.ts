...
export function fadeAudioSignalInPlace(
	samples: Float64Array,
	fadeMargin: number,
	windowFunction: WindowFunctions.WindowFunction
) {
...
export function genSpectrum(
	...
	const windowedSamples = WindowFunctions.applyWindowById(
		evenSamples,
		windowFunctionId
	);
...
export function findMaxFunctionValue(
	f: (x: number) => number,
	xVals: number[]
): number {
	let max = -Infinity;
